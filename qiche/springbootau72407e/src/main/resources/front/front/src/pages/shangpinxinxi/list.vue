...
<div>
	...
	<div class="list-preview" :style='{"padding":"0","margin":"0px auto","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}'>
		...
		<div class="category-3" :style='{"border":"1px solid #eee","padding":"26px 20px 0","boxShadow":"0px 0px 20px #eee","margin":"20px auto 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"86%","height":"auto","order":"3"}'>
			...
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"50px","margin":"0 5px 0 0","objectFit":"cover","display":"block","height":"50px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px 7% 20px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
        ...
        <el-input v-model="formSearch.shangpinmingcheng" placeholder="汽车名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>商家名称：</div>
        <el-input v-model="formSearch.shangjiamingcheng" placeholder="商家名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	  ...
	  <el-button v-if="btnAuth('shangpinxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"0px","background":"#333","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/shangpinxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"width":"100%","padding":"0 7%","margin":"20px auto 10px","textAlign":"right","background":"#fff","order":"3"}'>
		<el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"0px","background":"#F5BB00"}' @click="sortClick('price')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}' class="icon iconfont icon-xiaoliang17" v-if="sortType!='price'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}' class="icon iconfont icon-xiaoliang17" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}' class="icon iconfont icon-xiaoliang17" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
		...
		<div class="list3 index-pv1" :style='{"padding":"0 0px","overflow":"hidden","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","clear":"both","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"1px solid #f6f6f6","width":"200px","padding":"0px","margin":"0 10px 0 0","overflow":"hidden","height":"200px"}' class="img-box">
				<img @click.stop="imgPreView(item.shangpintupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.shangpintupian && item.shangpintupian.substr(0,4)=='http'" :src="item.shangpintupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0","flex":"1"}'>
				<div class="title">{{item.shangpinmingcheng}}</div>
				<div class="title">{{item.shangpinzhonglei}}</div>
				<div class="title">{{item.shangjiamingcheng}}</div>
				<div v-if="item.price" class="price"><span :style='{"fontSize":"12px"}'>￥</span>{{item.price}}</div>
		        <div :style='{"padding":"2px 10px","borderColor":"#f0f0f0","borderStyle":"dashed","borderWidth":"0 0 1px 0"}' class="time">
	...
	<!-- 热门信息 -->
	<div class="hot" :style='{"width":"100%","padding":"0 7%","margin":"20px 0 0","background":"#fff","height":"auto","order":"5"}'>
	  ...
	  <div :style='{"width":"100%","padding":"20px","background":"#fff","justifyContent":"space-between","display":"flex","height":"auto"}'>
	    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"23%","background":"#fff","height":"auto"}' @click="toDetail(item)">
	      <img :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}' v-if="item.shangpintupian && item.shangpintupian.substr(0,4)=='http'" :src="item.shangpintupian.split(',')[0]" alt="">
	      <img :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}' v-else :src="baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","borderColor":"#eee","overflow":"hidden","whiteSpace":"nowrap","color":"#666","textAlign":"center","borderWidth":"0 0 1px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.shangpinmingcheng}}</div>
	      <div :style='{"padding":"0 10px","borderColor":"#eee","overflow":"hidden","whiteSpace":"nowrap","color":"#666","textAlign":"center","borderWidth":"0 0 1px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.shangpinzhonglei}}</div>
	      <div :style='{"padding":"0 10px","borderColor":"#eee","overflow":"hidden","whiteSpace":"nowrap","color":"#666","textAlign":"center","borderWidth":"0 0 1px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.shangjiamingcheng}}</div>
	      <!--<div :style='{"padding":"0 10px","lineHeight":"32px","fontSize":"14px","color":"#999","textAlign":"right"}'>2022-02-02</div>-->
	    </div>
	  </div>
	</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('shangpinzhonglei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'shangpinzhonglei'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.shangjiamingcheng != '') searchWhere.shangjiamingcheng = '%' + this.formSearch.shangjiamingcheng + '%';
