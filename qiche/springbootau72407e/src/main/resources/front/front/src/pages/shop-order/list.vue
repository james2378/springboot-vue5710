...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    <div class="section-title" :style='{"margin":"10px 0","color":"#fff","textAlign":"center","background":"#333","width":"100%","fontSize":"20px","lineHeight":"54px"}'>我的订单</div>
    ...
    <el-table :data="tableData" style="width: 100%" >
      ...
      <el-table-column label="汽车" align="center" width="200px">
        <template slot-scope="scope">
          <div class="shangpin" v-if="scope.row.id">
...
export default {
    ...
    methods: {
		getBackPrice(row){
			let price = Number(row.buynumber * row.price)
                ...
                this.$http.get(item.tablename+'/info/'+item.goodid, {}).then(res => {
                    ...
                    this.$http.get(table+'/session', {}).then(res => {
                        ...
                        if(item.type==1||item.type==3) {
                            // 如果该汽车存在积分
                            if (good.jf) {
                                this.user.jf = (Number(this.user.jf) - Number(this.getBackPrice(item))).toFixed(2);
                            }
                            this.user.money = (Number(this.user.money) + Number(this.getBackPrice(item))).toFixed(2);
                            // 更新用户余额
                            ...
                            this.user.jf = Number(this.user.jf) + Number(this.getBackPrice(item));
                            // 更新用户余额
                            this.$http.post(table+'/update', this.user).then(res => {
                                item.status = '已退款'
								localStorage.setItem('sessionForm',JSON.stringify(this.user))
