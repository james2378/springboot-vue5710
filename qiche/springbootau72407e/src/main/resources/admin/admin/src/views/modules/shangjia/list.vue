<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#555","borderRadius":"0px","background":"rgba(255,255,255,1)","width":"100%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">商家号</label>
						<el-input v-model="searchForm.shangjiahao" placeholder="商家号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">商家名称</label>
						<el-input v-model="searchForm.shangjiamingcheng" placeholder="商家名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10px","fontSize":"14px","color":"inherit","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('shangjia','商家数量统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"padding":"0","borderColor":"#eee","borderRadius":"0px","background":"none","borderWidth":"1px 0 0 1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shangjiayouxiang"
						label="商家邮箱">
						<template slot-scope="scope">
							{{scope.row.shangjiayouxiang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="yingyezhizhao" label="营业执照">
						<template slot-scope="scope">
							<el-button v-if="scope.row.yingyezhizhao" type="text" size="small" @click="download(scope.row.yingyezhizhao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="shangpuzhaopian" width="200" label="商铺照片">
						<template slot-scope="scope">
							<div v-if="scope.row.shangpuzhaopian">
								<img v-if="scope.row.shangpuzhaopian.substring(0,4)=='http'" :src="scope.row.shangpuzhaopian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.shangpuzhaopian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('shangjia','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shangjiamingchengChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var shangjiamingchengChart1 = echarts.init(document.getElementById("shangjiamingchengChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                shangjiamingchengChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shangjiamingchengChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.shangjiahao!='' && this.searchForm.shangjiahao!=undefined){
            params['shangjiahao'] = '%' + this.searchForm.shangjiahao + '%'
          }
