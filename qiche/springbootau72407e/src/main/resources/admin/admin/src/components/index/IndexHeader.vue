<template>
	<div class="navbar">
		<div class="title" :style='{"display":"block","order":"1"}'>
			...
			<span class="title-name" :style='{"padding":"0 0 0 12px","lineHeight":"44px","fontSize":"18px","color":"#FFFF99","float":"left","fontWeight":"500"}'>{{this.$project.projectName}}</span>
		</div>
		<!--
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(sessionTable == 'shangjia') {
						this.$storage.set('headportrait',data.data.shangpuzhaopian)
					}
