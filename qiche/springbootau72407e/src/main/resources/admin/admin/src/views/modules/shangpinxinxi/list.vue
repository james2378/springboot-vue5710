<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#555","borderRadius":"0px","background":"rgba(255,255,255,1)","width":"100%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">汽车名称</label>
						<el-input v-model="searchForm.shangpinmingcheng" placeholder="汽车名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10px","fontSize":"14px","color":"inherit","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('shangpinxinxi','汽车统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						汽车统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('shangpinxinxi','汽车种类统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						汽车种类统计
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"0","borderColor":"#eee","borderRadius":"0px","background":"none","borderWidth":"1px 0 0 1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shangpinzhonglei"
						label="汽车种类">
						<template slot-scope="scope">
							{{scope.row.shangpinzhonglei}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="shangpintupian" width="200" label="汽车图片">
						<template slot-scope="scope">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shangpinzhongleiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var shangpinzhongleiChart2 = echarts.init(document.getElementById("shangpinzhongleiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                shangpinzhongleiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shangpinzhongleiChart2.resize();
                };
            }
        });
      })
